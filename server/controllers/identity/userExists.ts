...
const logger = getLogger('controllers - userExists');
logger.level = config.get('logLevel');

const userExists = async (req: express.Request, res: express.Response) => {
  logger.debug('entering >>> userExists()');
  ...
  try {
    ...
    const invokeResponse = await contract.evaluateTransaction('userExists', key);
  ...
  logger.debug('exiting <<< userExists()');
  util.sendResponse(res, jsonRes);
};

export { userExists as default };
